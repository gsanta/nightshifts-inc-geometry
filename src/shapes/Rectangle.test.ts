...
describe('Rectangle', () => {
    describe ('getNarrowSides', () => {
        it ('returns with the two narrower sides out of the four sides of the rectangle [WIDTH is bigger]', () => {
            ...
            expect(_.some(narrowSides, new Segment(new Point(5, 0), new Point(5, 2))), 'Point(5, 0), Point(5, 2) not found').to.be.true;
            expect(_.some(narrowSides, new Segment(new Point(0, 0), new Point(0, 2))), 'Point(0, 0), Point(0, 2) not found').to.be.true;
        });
        ...
        it ('returns with the two narrower sides out of the four sides of the rectangle [HEIGHT is bigger]', () => {
            ...
            expect(_.some(narrowSides, new Segment(new Point(0, 0), new Point(2, 0))), 'Point(0, 0), Point(2, 0) not found').to.be.true;
            expect(_.some(narrowSides, new Segment(new Point(0, 5), new Point(2, 5))), 'Point(0, 5), Point(2, 5) not found').to.be.true;
        });
    ...
    describe('`getCenterLines`', () => {
        ...
        expect(centerLines[0]).to.eql(new Segment(new Point(3.5, -3), new Point(3.5, 1)));
        expect(centerLines[1]).to.eql(new Segment(new Point(1, -1), new Point(6, -1)));
    });
});